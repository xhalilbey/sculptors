 ...
 *
 * Services in lib/workos, lib/auth and lib/security import this barrel and
 * nothing deeper; the ESLint boundary keeps drizzle-orm, pg and src/db out of
 * everything else. A service holds an `IdentityDb` only to hand it back to a
 * repository, which is what lets several writes share one transaction. The
 * handle is opaque (internal/handle.ts): a service cannot run a query with it.
 */
...
export type { MirroredMembership, MembershipUpsert } from './repositories/memberships.repository';
export type { UserSnapshot, WorkOSProfile } from './repositories/users.repository';
export type { OrganizationPatch } from './repositories/organizations.repository';
export type { MembershipRow, OrganizationRow, UserRow } from '@/db/schema';
export {
  isMembershipId,
  isOrganizationId,
  isUserId,
